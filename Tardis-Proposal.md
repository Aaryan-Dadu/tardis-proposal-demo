...
* When the config will get merged, a setup.yaml will be generated which particularly specifies the required details like the tardis version to be used for the config, the author, the date, and installation instructions for that tardis version.  
...
* The updated or added configs are listed and then for them a setup.yaml will be generated using a script similar to the script in First Objective PR. This setup.yaml contains instructions for installing and setting up the current version of tardis.  
...
* The script will inject the configs, atomic data, etc to the template notebook we have in the repo. The notebook will run and it will get saved in the out/ of each config folder or to a common out/ folder.  
...
* The first is many configs has local path to atomic data file, but this can be resolved without altering the config by injecting the atom data file in the run\_tardis based upon the file name in the config.  
* Like the setups in the tardis-setups/integration\_tests/ has the key black\_body\_sampling which is deprecated, few configs like 2022/2019eix/29d.yml and few others have shape mismatch error. So, in some cases where the tests would fail we can either leave notebook generation for them or  we can specify the version of tardis for which they would be compatible.  
